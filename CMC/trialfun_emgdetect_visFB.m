function [trl] = trialfun_emgdetect_visFB(cfg)
...
%                     'chanindx', chanindx, 'checkboundary', false);
    ...
    % plotting
        ...
        [peaks_art, locs_art] = findpeaks(emgstd(hdr.Fs*cfg.shift+1:(length(emgstd)-hdr.Fs*cfg.shift)),...
            'MinPeakHeight', 10, 'MinPeakDistance',hdr.Fs*cfg.shift*(1-cfg.overlap));
    
        [peaks_all, locs_all] = findpeaks(emgstd(hdr.Fs*cfg.shift+1:(length(emgstd)-hdr.Fs*cfg.shift)),...
            'MinPeakHeight', cfg.thresh, 'MinPeakDistance',hdr.Fs*cfg.shift*(1-cfg.overlap));
        
        [locs, idx] = setdiff(locs_all, locs_art);
        peaks = peaks_all(idx);
        locs=locs+499;
        ...
        %plot(setdiff(locs,locs_art)./hdr.Fs, peaks(idx), '*k')
        plot(time_vector,emgstd, 'b'), hold on, plot(locs./hdr.Fs, peaks, '*r');
        plot([0 time_vector(end)], [thresh thresh], '--m'), xlabel('Time(s)'), ylabel('z-trans(hilb(EMG))')
        legend('EMG-hilbert-std', 'Peaks', 'Threshold')
        title('Full data')
